  ...
  >
    <div class="switchColumnGrid">
      ...
      <div class="switchColumn">
        ...
        <FtToggleSwitch
          v-if="usingAndroid"
          :label="t('Settings.Player Settings.Enter Picture in Picture When Leaving App')"
          :compact="true"
          :default-value="autoEnterPipOnLeave"
...
/** @type {import('vue').ComputedRef<boolean>} */
const autoEnterPipOnLeave = computed(() => store.getters.getAutoEnterPipOnLeave)
...
const defaultViewingMode = computed(() => {
  ...
  if ((defaultViewingMode === 'external_player' && (!process.env.IS_ELECTRON || externalPlayer.value === '')) ||
    (!process.env.IS_ELECTRON && (defaultViewingMode === 'fullscreen' || defaultViewingMode === 'fullscreen_always_on')) ||
    (!process.env.IS_ELECTRON && !process.env.IS_ANDROID && defaultViewingMode === 'pip')) {
    return 'default'
  }
