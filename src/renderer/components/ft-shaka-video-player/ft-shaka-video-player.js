...
import { AndroidPipButton } from './player-components/AndroidPipButton'
...
export default defineComponent({
  ...
  setup: function (props, { emit, expose }) {
    ...
    /** @type {import('vue').ComputedRef<boolean>} */
    const autoEnterPipOnLeave = computed(() => {
    ...
    onAppPause = () => {
      if (process.env.IS_ANDROID && (document.body.classList.contains('androidPip') || isInPictureInPicture())) {
        return
      }
      try {
     ...
     */
    function getAndroidPipAspectRatio() {
      const videoEl = video.value
      const width = videoEl?.videoWidth || 16
      const height = videoEl?.videoHeight || 9
      return { width, height }
    }
    ...
    const uiConfig = computed(() => {
      ...
      if (onlyUseOverFlowMenu.value) {
        uiConfig.overflowMenuButtons = [
          ...
          'loop',
          'ft_screenshot',
          process.env.IS_ANDROID ? 'ft_android_pip' : 'picture_in_picture',
          'ft_full_window',
          'recenter_vr',
          'toggle_stereoscopic',
        ]
      ...
      if (props.format === 'audio') {
        ...
        removeFromArrayIfExists(elementList, 'ft_android_pip')
      }

      if (process.env.IS_ANDROID && props.format !== 'audio') {
        // Keep system PiP one tap away even when the rest of the controls are in the overflow menu
        removeFromArrayIfExists(uiConfig.overflowMenuButtons, 'ft_android_pip')
        if (!uiConfig.controlPanelElements.includes('ft_android_pip')) {
          const fullscreenIndex = uiConfig.controlPanelElements.indexOf('fullscreen')
          if (fullscreenIndex === -1) {
            uiConfig.controlPanelElements.push('ft_android_pip')
          } else {
            uiConfig.controlPanelElements.splice(fullscreenIndex, 0, 'ft_android_pip')
          }
        }
      }
    ...
    function handleCanPlay() {
      ...
      if (startInPip && props.format !== 'audio') {
        if (process.env.IS_ANDROID) {
          startInPip = false
          if (!document.body.classList.contains('androidPip')) {
            enterAndroidPictureInPicture()
          } else {
            syncAndroidPipState()
          }
        } else if (ui.getControls().isPiPAllowed() && process.env.IS_ELECTRON) {
          startInPip = false
    ...
    function registerAndroidPipButton() {
      events.addEventListener('enterAndroidPip', () => {
        enterAndroidPictureInPicture()
      })
       ...
       * @implements {shaka.extern.IUIElement.Factory}
       */
      class AndroidPipButtonFactory {
        create(rootElement, controls) {
          return new AndroidPipButton(events, rootElement, controls)
        }
      }

      shakaControls.registerElement('ft_android_pip', new AndroidPipButtonFactory())
      shakaOverflowMenu.registerElement('ft_android_pip', new AndroidPipButtonFactory())
    }
    ...
    onBeforeUnmount(() => {
      ...
      window.removeEventListener('pip-enter', handleAndroidPipEnter)
      ...
      if (ui) {
        if (ui.getControls()) {
          ...
          uiState = {
            ...
            startNextVideoInPip: process.env.IS_ANDROID
              ? (document.body.classList.contains('androidPip') || isInPictureInPicture())
              : controls.isPiPEnabled()
          }
        }
